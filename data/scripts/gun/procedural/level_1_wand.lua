...
function do_level1( level )
	...
	local always_cast_frequency = ModSettingGet("custom_always_cast.frequency")
	if( Random(1,100) < always_cast_frequency ) then
		dofile("data/scripts/gun/gun_actions.lua")
		local mod_actions = {}
		for i, action in ipairs(actions) do
			local enabled = ModSettingGet("custom_always_cast.action_" .. action.id)
			if(enabled) then
				table.insert(mod_actions, action.id)
			end
		end
		if( #mod_actions > 0) then
			local random_index = Random(1, #mod_actions)
			local action = mod_actions[random_index]
			AddGunActionPermanent( entity_id, action )
		end
	end
end
do_level1( 1 )